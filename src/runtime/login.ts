...
function defaultRunLogin(env: NodeJS.ProcessEnv): number | null {
...
function createLoginRuntime(poolHome: string): string {
...
export function loginAccount(options: LoginAccountOptions): AddAccountResult {
  ...
  try {
    ...
    try {
      ...
      throw new AccountStoreError(
        ...
        "登录流程已结束，但临时目录清理失败；请勿继续切换账号，并手动检查 Codex Pool runtime 目录",
      );
    }
  }
}
