...
- 在列表、刷新和切换前对比全局 `auth.json` 与账号池的账号指纹：外部登录到已保存账号时原子同步最新凭证并更新 `active-account`，未导入账号时清空当前标记但不自动创建账号；
...
- 记录菜单栏 CLI 操作的脱敏诊断日志；额度查询等待 app-server 完全退出，超时时强制终止，再删除临时 `CODEX_HOME`，并在查询前清理同账号的历史遗留运行目录；
- 不尝试修改已运行 App 的账号状态。
...
1. 添加 A，列表能看到账号邮箱和套餐；
2. Codex App 运行时点击菜单栏“导入当前账号”，直接以当前邮箱作为别名完成导入；
3. 登录 B，列表能看到 A/B；
4. 在 B 卡片菜单中重命名为 `company`，列表和切换入口同步更新；
5. 退出 App 后切换到 B；
6. 重新打开 App，确认 Profile 显示 B；
7. 使用 B 一次任务；
8. 再次退出 App 并切回 A；
9. 确认 A 仍然可以正常登录和运行；
10. 在 `company` 卡片菜单中确认永久删除，列表和账号仓库不再保留该账号凭证；
11. 验证共享项目、会话和插件未被误删。
