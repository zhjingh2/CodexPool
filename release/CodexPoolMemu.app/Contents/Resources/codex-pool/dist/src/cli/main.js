#!/usr/bin/env node
import { AccountStoreError, addCurrentAccount, listAccounts, purgeAccount, renameAccount, } from "../account-store/index.js";
...
Usage:
  ...
  codex-pool --version
`;
function renderAccounts(accounts, json) {
    if (json) {
    ...
    for (const account of accounts) {
        ...
        const usage = account.usageStatus === "available"
            ...
            : account.usageStatus === "unavailable"
                ? "暂不可用"
                : "未查询";
        lines.push(`${account.alias}\t${plan}\t${status}\t${credential}\t${usage}\t${account.current ? "✓" : "-"}\t${quota}\t${reset}`);
    }
    return `${lines.join("\n")}\n`;
}
...
async function main(args) {
    ...
    if (command === "account") {
        ...
        try {
            ...
            process.stdout.write(accountOptions.includes("--json")
    ...
    if (command === "switch") {
        ...
        try {
            ...
            if (error instanceof AccountStoreError) {
                process.stderr.write(error.code === "APP_LAUNCH_FAILED"
