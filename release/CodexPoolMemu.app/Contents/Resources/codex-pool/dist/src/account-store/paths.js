...
const ACCOUNT_ALIAS_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._+@-]{0,127}$/u;
const RESERVED_ALIASES = new Set([".", "..", "runtime", "accounts"]);
export function validateAccountAlias(alias) {
    const normalized = alias.trim();
    if (!ACCOUNT_ALIAS_PATTERN.test(normalized) || RESERVED_ALIASES.has(normalized)) {
        throw new AccountStoreError("INVALID_ALIAS", "账号别名必须为 1-128 个字母、数字、点、下划线、加号、@ 或连字符，并且不能包含路径分隔符");
    }
    return normalized;
}
export function resolvePoolHome(env = process.env, userHome = homedir()) {
    const configured = env.CODEX_POOL_HOME?.trim();
    return configured ? resolve(configured) : join(userHome, ".codex-pool");
}
export function getAccountDirectory(poolHome, alias) {
