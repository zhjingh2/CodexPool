...
import { acquirePoolLock, assertRegularPrivateSourceFile, writePrivateFileAtomically, } from "./files.js";
import { getAccountDirectory } from "./paths.js";
import { resolveCodexHome } from "../preflight/doctor.js";
function readGlobalAuth(codexHome) {
 ...
 */
export function reconcileCurrentAccount(options) {
    ...
    const matched = options.accounts.find((account) => account.accountFingerprint === globalAuth.fingerprint);
    ...
    const targetAuthPath = join(getAccountDirectory(options.poolHome, matched.alias), "auth.json");
