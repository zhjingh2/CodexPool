import { existsSync, lstatSync, readFileSync, rmSync, renameSync, } from "node:fs";
...
function readActiveAlias(activePath) {
...
function writeRenamedMetadata(path, metadata, alias) {
    writePrivateFileAtomically(path, `${JSON.stringify({ ...metadata, alias }, null, 2)}\n`);
}
export function renameAccount(options) {
    ...
    try {
        ...
        try {
            ...
            throw new AccountStoreError("RENAME_ROLLBACK_FAILED", `账号重命名失败且回滚失败，请保留账号仓库并检查 ${poolHome}`);
        }
        throw error;
    }
    finally {
...
export function purgeAccount(options) {
    const alias = validateAccountAlias(options.alias);
    if (options.confirmation !== alias) {
        throw new AccountStoreError("PURGE_CONFIRMATION_REQUIRED", `永久删除账号前必须精确输入账号别名：${alias}`);
    }
    const env = options.env ?? process.env;
    const poolHome = resolvePoolHome(env, options.userHome ?? homedir());
    ...
    try {
        ...
        if (activeAlias === alias) {
            throw new AccountStoreError("CURRENT_ACCOUNT_PROTECTED", `账号 ${alias} 是当前激活账号，请先切换到其他账号后再 purge`);
        }
        try {
            rmSync(accountDirectory, { recursive: true, force: false });
        }
        catch (error) {
            throw new AccountStoreError("PURGE_FAILED", `账号 ${alias} 删除失败：${error instanceof Error ? error.message : "未知文件系统错误"}`);
        }
        if (pathExists(accountDirectory)) {
