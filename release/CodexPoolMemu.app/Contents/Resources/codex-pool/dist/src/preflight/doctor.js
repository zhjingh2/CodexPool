...
function checkAppServerSchema(dependencies, codexEnv) {
    const schemaDirectory = dependencies.makeTempDir("codex-pool-doctor-");
    try {
        const result = dependencies.run("codex", ["app-server", "generate-json-schema", "--experimental", "--out", schemaDirectory], codexEnv);
        if (result.status !== 0) {
...
export function runDoctor(dependencies = createDefaultDoctorDependencies()) {
    const checks = [];
    ...
    checks.push({
        ...
        summary: dependencies.platform === "darwin"
            ? "macOS"
    ...
    if (codexAvailable && authExists) {
        ...
        checks.push({
            ...
            summary: loginResult.status === 0
                ? loginResult.stdout.trim() || "已登录"
                : loginResult.error ?? (loginResult.stderr.trim() || "登录状态不可用"),
        });
    }
    const processResult = dependencies.run("ps", ["-axo", "pid=,ppid=,command="], dependencies.env);
